...
import { VASTU_SERVICES } from '../../data/servicesData';
...
interface VastuServicesSectionProps {
  onOpenConsultationModal: (serviceName?: string) => void;
}

export const VastuServicesSection: React.FC<VastuServicesSectionProps> = ({
  ...
  return (
    <section id="vastu" className="py-16 lg:py-24 bg-[#080A14] relative border-t border-[#C8A45D]/15">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="text-center max-w-2xl mx-auto space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#10152A] border border-[#C8A45D]/30 text-xs font-mono text-[#E8D5A8] uppercase tracking-wider">
            <span>Spatial & Architectural Harmony</span>
          </div>
          ...
          <p className="text-xs sm:text-sm text-[#9EA3B5]">
            100% Non-demolition spatial remedies for residences, corporate offices, and commercial properties.
          </p>
        </div>

        {/* Master Vastu Services Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {VASTU_SERVICES.map(service => (
            <div
              key={service.id}
              className="flex flex-col justify-between p-5 rounded-xl bg-[#10152A]/90 border border-[#C8A45D]/20 hover:border-[#C8A45D]/60 transition-all hover:shadow-[0_8px_30px_rgba(200,164,93,0.12)] group"
            >
